	...
	class WPF_EDD extends WPF_Common {
		...
		*/
		public function __construct() {
			...
			$this->checkout   = isset( $edd_options['wpf_custom_checkout'] ) ? $edd_options['wpf_custom_checkout'] : $this->checkout;
			$this->secret_key = isset( $edd_options['wpf_secret_key'] ) ? $edd_options['wpf_secret_key'] : null;
			$this->public_key = isset( $edd_options['wpf_public_key'] ) ? $edd_options['wpf_public_key'] : null;
		...
		*/
		...
		public function plugin_action_links( $links ) {
			...
			$plugin_links = array(
				sprintf( __( '<a href="%s">Settings</a>', $this->slug ), admin_url( 'edit.php?post_type=download&page=edd-settings&tab=gateways' ) ),
			);
		...
		*/
		public function settings( $settings ) {
			...
			$wpf_settings = array(
				...
				array(
					'id'    => 'wpf_billing',
					'name'  => __( 'Checkout Billing', $this->slug ),
					'desc'  => __( 'Require customers to include billing information when entering a card during the wpFortify checkout.', $this->slug ),
					'type'  => 'checkbox'
				),
				array(
		...
		*/
		public function process_payment( $purchase_data ) {
			...
			if( !isset( $this->secret_key ) || !isset( $this->public_key ) ) {

				edd_set_error( 0, sprintf( __( 'Please enter the wpFortify access keys in <a href="%s">settings</a>.', $this->slug ), admin_url( 'edit.php?post_type=download&page=edd-settings&tab=gateways' ) ) );
			...
			if( $payment ) {
				...
				$wpf_charge = array (
					'wpf_charge' => array(
						...
						'action'       => 'charge_card',
						'site_title'   => isset( $edd_options['wpf_title'] ) ? $edd_options['wpf_title'] : get_bloginfo(),
						'site_url'     => get_bloginfo( 'url' ),
						'listen_url'   => get_bloginfo( 'url' ) . '/?' . $this->slug . '=callback',
						'return_url'   => get_permalink( $edd_options['success_page'] ),
						'cancel_url'   => get_permalink( $edd_options['failure_page'] ),
						'image_url'    => isset( $edd_options['wpf_checkout_image'] ) ? $edd_options['wpf_checkout_image'] : '',
						'customer_id'  => '',
						'card_id'      => '',
						...
						'button'       => $button,
						'currency'     => edd_get_currency(),
						'testmode'     => isset( $edd_options['test_mode'] ) ? true : false,
						'capture'      => true,
						'billing'      => isset( $edd_options['wpf_billing'] ) ? true : false,
						'metadata'     => array(
							'order_id' => $payment
						)
					)
